...
target("Yukino")
    ...
    on_load(function (package)
        local include_path = path.join(get_config("Yukino_inc"), "Yukino")
    ...
    after_build(function (target)
        local lib_dir = get_config("Yukino_lib")
    ...
    on_install(function (target)
        os.mkdir(path.join(target:installdir(), "include/Yukino"))
        os.mkdir(path.join(target:installdir(), "lib"))
        os.cp(path.join(get_config("Yukino_inc"), "Yukino"), path.join(target:installdir(), "include"))
        shared_suffix = "*.so"
        if is_plat("macosx") then
            shared_suffix = "*.dylib"
        end
        if target:is_static() then
            os.cp(path.join(get_config("Yukino_lib"), "*.a"), path.join(target:installdir(), "lib"))
        else
            os.cp(path.join(get_config("Yukino_lib"), shared_suffix), path.join(target:installdir(), "lib"))
        end
    end)
